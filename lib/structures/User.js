...
class User extends Base {
    ...
    */
    ...
    /**
     * Get the member object for a guild if the user is in that guild
     * @arg {String} guildID The guild you want to check for
     * @returns {Member|null}
     */
    memberForGuild(guildID) {
        var guild = this._client.guilds.find(g => g.id == guildID);
        if (guild) {
            var member = guild.members.find(m => m.id == this.id);
    ...
    /**
     * Check if the user is part of a guild
     * @arg {String} guildID The guild you want to check for
     * @returns {Boolean}
     */
    isMember(guildID) {
        return (this.memberForGuild(guildID)) ? true : false;
    }
}
